...
env = ZEEK_DEFAULT_CONNECT_RETRY=5 BROKER_FILE_VERBOSITY=debug

[worker]
role = worker
scripts = testing/sender.zeek
interface = lo
env = ZEEK_DEFAULT_CONNECT_RETRY=5
EOF
...
try_until controller_cmd "cat $worker_logs/telemetry.log " \
    "| grep -E 'zeek_broker_backpressure_overflows_total.+worker,proxy.+1.0'" \
    || fail "telemetry did not report backpressure overflow"
